def const_los_Acc_td(v0, acc, **params):
    ...
    dt1 = np.full((len(t1),), 1/dt, dtype=float)
    dt2 = np.full((len(t2),), 1/dt, dtype=float)
    ...
    t_val = np.arange(np.min(t_new), np.max(t_new), 1/dt)
    hp_interp = np.interp(t_val, t_new, hp)
    hc_interp = np.interp(t_val, t_new, hc)
    hp_new = TimeSeries(hp_interp, delta_t=1/dt)
    hc_new = TimeSeries(hc_interp, delta_t=1/dt)
